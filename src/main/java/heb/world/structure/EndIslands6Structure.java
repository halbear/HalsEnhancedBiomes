...
public class EndIslands6Structure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 25000) {
						...
						for (int a = 0; a < count; a++) {
							...
							BlockPos spawnTo = new BlockPos(i + -6, j + 21, k + -6);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("heb", "end1_13x25_island5"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("end_islands_6"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("heb:end_islands_6"), configuredFeature);
		}
	}
