...
public class AbandonedTrainSurfaceMesaStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
			feature = new Feature<NoFeatureConfig>(NoFeatureConfig.field_236558_a_) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if (!dimensionCriteria)
						return false;
					if ((random.nextInt(1000000) + 1) <= 10500) {
						...
						for (int a = 0; a < count; a++) {
							...
							if (blockAt.getBlock() == Blocks.COARSE_DIRT)
								blockCriteria = true;
							if (blockAt.getBlock() == Blocks.WHITE_TERRACOTTA)
								blockCriteria = true;
							...
							BlockPos spawnTo = new BlockPos(i + -1, j + 1, k + -4);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("heb", "badlands3x4x8abandonedtrain"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("abandoned_train_surface_mesa"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("heb:abandoned_train_surface_mesa"), configuredFeature);
		}
	}
