...
public class GiantSpruceTreeSurfaceStructure {
	...
	private static class FeatureRegisterHandler {
		...
		public static void registerFeature(RegistryEvent.Register<Feature<?>> event) {
				...
				public boolean generate(ISeedReader world, ChunkGenerator generator, Random random, BlockPos pos, NoFeatureConfig config) {
					...
					if ((random.nextInt(1000000) + 1) <= 300000) {
						...
						for (int a = 0; a < count; a++) {
							...
							BlockPos spawnTo = new BlockPos(i + -8, j + 1, k + -8);
							int x = spawnTo.getX();
							int y = spawnTo.getY();
							int z = spawnTo.getZ();
							Template template = world.getWorld().getStructureTemplateManager()
									.getTemplateDefaulted(new ResourceLocation("heb", "spruce17x42_sprucetree"));
							if (template == null)
								return false;
			...
			event.getRegistry().register(feature.setRegistryName("giant_spruce_tree_surface"));
			Registry.register(WorldGenRegistries.CONFIGURED_FEATURE, new ResourceLocation("heb:giant_spruce_tree_surface"), configuredFeature);
		}
	}
